...
var if_nextpage = 1
var if_total_pages = 1
var if_additional_parameters = ""
var if_url = ""
...
function add_more_content(ajax) {
    img_div.insert(ajax.responseText)
    refresh()
    img_div.childNodes.item(array_images[new_caption_id]).go_to_this_image()
    $('loaded_rips_count').update(array_images.length)
    if_is_loading = false
}
/** ajax-preload end **/
...
{	
   ...
   /** ajax-preload **/
    if(!if_is_loading && new_caption_id > $$('#images img').length - 30) {
        if_is_loading = true
        get_more_content()
    }
    /** ajax-preload end **/
...
function refresh(onload)
{
    ...
    { 
        ...
        if (image.nodeType == 1)
        {
            ...
            /* ajax-preload */
            image.go_to_this_image = function() { glideTo(this.x_pos, this.i); }
            /* ajax-preload  end */
